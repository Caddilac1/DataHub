...
def customer_required(view_func, denied_url="home"):
    @functools.wraps(view_func)
    def wrapper(request, *args, **kwargs):
        if (request.user.is_authenticated and request.role == "customer") or (request.user.is_authenticated and request.user.role == "admin"):
            return view_func(request, *args, **kwargs)
...
def agent_required(view_func, denied_url="home"):
    @functools.wraps(view_func)
    def wrapper(request, *args, **kwargs):
        if (request.user.is_authenticated and request.user.role == "agent") or (request.user.is_authenticated and request.user.role == "admin"):
            return view_func(request, *args, **kwargs)
        messages.info(request, "Page not found")
        return redirect(denied_url)  
    return wrapper
